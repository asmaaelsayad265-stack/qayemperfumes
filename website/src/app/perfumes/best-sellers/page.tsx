  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        setLoading(true);
        setError(null);
        const data = await retryWithBackoff(() => productsApi.getBestSellers());
        setProducts(data);
      } catch (err) {
      ...
      <section>
        <div className="flex items-end justify-between gap-4">
          <div className="space-y-1">
            <h1 className="text-2xl font-extrabold">الأكثر مبيعًا</h1>
    ...
    return (
      <section>
        ...
        <div className="rounded-3xl border border-gold/15 bg-surface/70 p-12 text-center">
          <p className="text-muted">لا توجد منتجات الأكثر مبيعاً حالياً.</p>
        </div>
      </section>
    );
  }
  ...
  return (
    <section>
      <div className="flex items-end justify-between gap-4">
        <div className="space-y-1">
          <h1 className="text-2xl font-extrabold">الأكثر مبيعًا</h1>
          <p className="text-sm text-muted">أفضل العطور اختياراً</p>
        </div>
...
export default function PerfumesBestSellersPage() {
  return (
    <PublicShell>
