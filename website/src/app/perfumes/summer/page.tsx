  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        setLoading(true);
        setError(null);
        const summerCategory = await retryWithBackoff(() => categoriesApi.getBySlug('summer'));
        if (summerCategory) {
          const data = await retryWithBackoff(() => productsApi.getByCategory(summerCategory.id));
          setProducts(data);
        }
      } catch (err) {
      ...
      <section>
        <div className="flex items-end justify-between gap-4">
          <div className="space-y-1">
            <h1 className="text-2xl font-extrabold">عطور صيفية</h1>
    ...
    return (
      <section>
        ...
        <div className="rounded-3xl border border-gold/15 bg-surface/70 p-12 text-center">
          <p className="text-muted">لا توجد عطور صيفية متاحة حالياً.</p>
        </div>
      </section>
    );
  }
  ...
  return (
    <section>
      <div className="flex items-end justify-between gap-4">
        <div className="space-y-1">
          <h1 className="text-2xl font-extrabold">عطور صيفية</h1>
          <p className="text-sm text-muted">انتعاش صيفي فاخر</p>
        </div>
...
export default function PerfumesSummerPage() {
  return (
    <PublicShell>
